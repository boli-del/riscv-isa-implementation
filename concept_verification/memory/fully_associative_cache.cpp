...
SC_MODULE(L1_CACHE){
    ...
    void process(){
        while(true){
            ...
            unsigned int tag_num = location.read() >> 6;
            if(rst_n.read()){
                out_state.write(1);
                l2_call.write(0);
                replacement.write(0);
                ...
                if(state_in.read() == 1){
                    ...
                    if(found){
                        if(valid[idx]){
                            if(w_enable){
                                dirty[idx] = 1;
                                first_mem[idx].range(((offset)*8) + 31, offset*8) = data_in;
                            }else{
                        ...
                        //fetch maximum locality / least recently used
                        for(int i = 0; i < 16; i++){
